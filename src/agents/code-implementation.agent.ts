...
export class CodeImplementationAgent extends BaseAgent<FeatureContext, ImplementationOutput> {
  ...
  private generateVfpFiles(comp: ProposedComponent): ImplementationFile[] {
    ...
    files.push({
      ...
      content: [
        ...
        `  PROCEDURE GetAll`,
        `    LOCAL lnResult`,
        `    lnResult = SQLEXEC(THIS.nHandle, "SELECT * FROM ${name}", "qResult")`,
        `    RETURN lnResult`,
        `  ENDPROC`,
        ``,
        `  PROCEDURE Insert(tcData)`,
        `    LOCAL lnResult`,
        `    lnResult = SQLEXEC(THIS.nHandle, "INSERT INTO ${name} (created_at) VALUES (GETUTCDATE())")`,
        `    RETURN lnResult`,
        `  ENDPROC`,
        ``,
        `  PROCEDURE Destroy`,
  ...
  private buildSetupInstructions(language: string, framework: string): string {
    ...
    if (lower.includes('foxpro')) return '1. Copy PRG files to project directory\n2. SET PROCEDURE TO the new files\n3. Verify SQL Server connection string';
